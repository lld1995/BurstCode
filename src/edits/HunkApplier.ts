  ...
  private async processFile(
    ...
    f.hunks.forEach((h, i) => {
      ...
      if (h.oldText === undefined && h.expectedReadVersion !== undefined) {
        ...
        // Only use the recovered oldText as an anchor when it is non-empty.
        // When oldText is '' (insertion at startLine=endLine+1, or a range of
        // blank lines), resolveOldTextHunk would silently treat it as an
        // insertion and discard the endLine — losing any deletion. Keeping
        // oldText undefined sends the hunk through the line-range path which
        // respects both startLine and endLine.
        if (oldText) {
          anchored = { ...h, oldText };
        }
      }
