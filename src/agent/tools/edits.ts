  ...
  const proposeEdit: Tool = {
    ...
    schema: {
      ...
      function: {
        name: 'propose_edit',
        description:
          "Queue edits across one or more files for user review. NON-BLOCKING: returns immediately. Edits are written to disk eagerly so the user can compile / run with them BEFORE accepting; Accept just confirms keeping them, Reject rolls the affected hunks back. Use for modifications to the user's existing source files. For agent-generated scripts or temp files you'll execute immediately, use write_file instead.\n\n" +
