...
export function buildEditTools(
  ...
  const proposeEdit: Tool = {
    ...
    async execute(args): Promise<ToolResult> {
      ...
      // If the model sends ranges[] without an explicit operation, infer the
      // mode: ranges with newText → replace_lines, otherwise → delete_lines.
      // This restores the pre-replace_lines convenience where ranges[] alone
      // triggered delete mode, while correctly routing ranges that carry
      // replacement text to replace_lines.
      const hasRanges = Array.isArray(rawRangesCandidate);
      const rangesImplyReplace = hasRanges && (
        (argsRecord as Record<string, unknown>).replacements !== undefined ||
        (argsRecord as Record<string, unknown>).replaceRanges !== undefined ||
        (rawRangesCandidate as Array<Record<string, unknown>>).some(
          (r) => r && typeof r === 'object' &&
            (r.newText ?? r.new_text ?? r.replacement ?? r.code ?? r.content ?? r.text) !== undefined
        )
      );
      const wantsDeleteLines =
        ['delete_lines', 'delete', 'delete_range', 'delete_ranges', 'remove_lines'].includes(operation) ||
        (hasRanges && operation === '' && !rangesImplyReplace);
