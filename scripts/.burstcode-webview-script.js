...
function replayLiveState(snap) {
  ...
  for (const t of tools) {
    ...
    sum.textContent = ' 27} ' + t.name + '(' + argSnippet + ') · running...';
    det.appendChild(sum);
    ...
    case 'tool-call-start': {
      ...
      if (toolElements.has(existingKey)) {
        ...
        if (!applyRichTool(existingDet, msg.payload.name, msg.payload.args, msg.payload.meta, null, false, false)) {
          if (existingSum) existingSum.textContent = ' 27} ' + msg.payload.name + '(' + JSON.stringify(msg.payload.args).slice(0, 200) + ') · running...';
        }
